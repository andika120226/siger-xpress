...
// ---------------------------------------------------------------------------
const TRAFFIC_CORRIDORS: { from: [number, number]; to: [number, number]; label: string; status: "congested" | "warning" }[] = [
  { from: [-5.4292, 105.2611], to: [-5.8708, 105.7533], label: "⚠ Macet: B.Lampung ↔ Bakauheni", status: "congested" },
  { from: [-4.8286, 104.8829], to: [-5.0345, 104.0754], label: "⚠ Macet: Kotabumi ↔ Liwa", status: "congested" },
  { from: [-5.1131, 105.3067], to: [-4.8286, 104.8829], label: "⚠ Renovasi: Metro ↔ Kotabumi", status: "warning" },
];
...
function DynamicSegment({ start, end, color, status, distance, startName, endName }: {
  ...
  const statusLabel = status === "congested" ? "Macet" : status === "warning" ? "Renovasi" : "Lancar";

  return (
    <Polyline positions={positions} pathOptions={{ color, weight: 6, opacity: 0.8 }}>
      <Tooltip sticky>
        <div className="text-xs">
          <p className="font-bold">{startName} &rarr; {endName}</p>
          <p>Status: <span className="uppercase font-semibold" style={{ color }}>{statusLabel}</span></p>
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      ...
      >
        ...
        {optimalSequence && trafficSegments && trafficSegments.map((seg, idx) => {
